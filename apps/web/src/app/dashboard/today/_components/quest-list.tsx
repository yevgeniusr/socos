  ...
  return (
    <section aria-labelledby="quests-heading" className="border-t border-outline-variant/25 pt-5">
      <div className="flex items-center justify-between gap-3">
        <h2 id="quests-heading" className="text-base font-black">Verified quests</h2>
        <span className="text-xs font-bold text-secondary">{pending.length} open</span>
      </div>
      {pending.length ? (
        <ul className="mt-3 space-y-2">
          {pending.map((quest) => (
            <li key={quest.questId} className="rounded-lg border border-outline-variant/30 bg-surface-container-low p-3">
